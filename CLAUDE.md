...
Repo **plugin `ai-dlc`** (kiêm marketplace — 1 repo duy nhất): đóng gói phương pháp **AI-DLC** (AI-Driven Development Lifecycle, theo white paper AWS — bản dịch nội bộ) thành Claude Code plugin cài được, có Control Tower, override per project và learning loop qua retro.
...
- **Ba luật v2 (chống sai từ gốc)**: stage 1 sinh `intent-plan.md` (Intent + Source Reading Plan + Provisional Unit Map, duyệt toàn văn tại Gate A) · **No-unread-source** (§4.8: mọi nguồn phải có trạng thái cuối + evidence trong `source-ledger.md`; còn `planned` là chặn Gate B/D) · **Unit ≤5h** (§4.9: ước lượng có breakdown; mỗi Unit đủ User Story · NFR · Rủi ro). Gate duyệt bằng tài liệu markdown đọc toàn văn trên Control Tower — approve mù bị chặn ở cả UI lẫn server.
