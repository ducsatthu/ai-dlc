...
This is a **documentation/research repository** (no code, no build) inside the `company-research/rd` monorepo. It captures how **AWS's AI-DLC (AI-Driven Development Lifecycle)** method would change the way tổ chức's teams divide and accept work, compared with their current Agile/Sprint and ticket-driven processes.

Primary content: `docs/ai-dlc-changes-adoption.md` — extracted from the slide deck `AI-DLC-changes-adoption.html` (source: AWS AI-DLC Method Definition). Documentation is written in Vietnamese with English technical terms kept as-is; follow that convention when editing or adding docs.
