...
2. **Project override THẮNG plugin default** — mỗi dự án custom bằng file override tại repo của mình, có hiệu lực ngay, không chờ plugin release. (Đúng pattern binding-rule PILOT đang dùng: *"Project override THẮNG generic default khi mâu thuẫn. Generic chỉ là fallback."*)
...
Luật vận hành (ghi trong hook SessionStart của plugin — bắt buộc như binding rule của PILOT):
...
**Luật cứng:** mọi agent tạo output (code, docs) PHẢI resolve đích qua map — *"không tự đoán path"* (kế thừa binding rule PILOT đang dùng). Map thiếu mục cần dùng → agent **hỏi**, không đoán.
...
|---|---|---|---|
| 1 | **Skeleton plugin + marketplace** — plugin.json, marketplace.json, cấu trúc thư mục, hook SessionStart nạp governance + binding rule; `/dlc-init` gồm bước dựng **workspace map** (scan repo → đề xuất → hỏi user chốt) | `/plugin install ai-dlc` chạy được; init trên PILOT ra map đúng (app-fe/, app-be/, wiki/docs/) | 0.5–1 ngày |
...
| 5 | **Pilot với PILOT Phase 2** — cài plugin vào PILOT, chạy intent thật tới Gate D, retro thật ra LL đầu tiên | LL thật đầu tiên được upstream vào 1.1.0 | theo tiến độ PILOT |
