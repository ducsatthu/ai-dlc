...
2. Tạo cây thư mục theo protocol §3 (gồm `context-memory/handoffs/`, `context-memory/session/log/` và `context-memory/escalations/` — chỗ cho phát hiện ngoài phạm vi unit, §4.13). Seed từ `${CLAUDE_PLUGIN_ROOT}/templates/`: `governance/dor.md`, `governance/dod.md`, `overrides/OVERRIDES.md`, `session/INDEX.md` (từ `context-index.md`); tạo rỗng: `governance/decisions-log.md`, `changelog.md`, `risks.md`, `tech-debt-register.md`.
